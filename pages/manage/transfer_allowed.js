...
class TransferAllowed extends Component {
  state = {
    mounted: false,
    headerHeight: 0,
    pausedHeight: 0,
    ...
    if (isValidAccount) {
      ...
      for (let i = 0; i < events.length; i++) {
        allowances[
          ...
          .allowance(events[i].returnValues.owner, currentAccount)
          .call();
      }
        ...
        <Table.Row key={key}>
          ...
          <Table.Cell>
            <Button
              fluid
              onClick={() => {
                this.setState({ senAddr: key, senAddrErr: false });
              }}
              style={{ overflow: 'auto' }}
            >
  ...
  onSubmit = async () => {
    ...
    try {
      if (this.state.senAddrErr || this.state.senAddr === '') {
      ...
      await cryptoByte.methods
        .transferFrom(
          web3.utils.toChecksumAddress(this.state.senAddr),
  ...
  render() {
    return (
      <Layout mounted={this.state.mounted}>
        <Head>
          <title>Crypto Byte - Transfer allowed</title>
        </Head>
        <MMPrompt />
        ...
        <Visibility onUpdate={this.updateContent}>
          ...
          >
            <Header as="h3" inverted dividing textAlign="center">
              You can transfer tokens, that you are allowed by other accounts
              with the form below.
            </Header>
              ...
              <Grid.Row columns={2}>
                <Grid.Column>
                  <Form inverted onSubmit={this.onSubmit}>
                    <Form.Input
                      error={this.state.senAddrErr}
                      label="Sender address (tokens will be transferred from this address)"
                    >
                      <Input
                        placeholder="0x0000000000000000000000000000000000000000"
                        value={this.state.senAddr}
                        onChange={event => {
                          this.setState({ senAddr: event.target.value });
                          ...
                          if (
                            ...
                            this.setState({ senAddrErr: false });
                          }
                        }}
                      />
                    </Form.Input>
                    ...
                    <Form.Input
                      ...
                      label="Receiver address (tokens will be transferred to this address)"
                    >
                      <Input
                        placeholder="0x0000000000000000000000000000000000000000"
                    ...
                    {this.state.amount &&
                      ...
                      !this.state.senAddrErr &&
                      !this.state.recAddrErr && (
                        <Message info style={{ overflow: 'auto' }}>
                          <p>
                            This is going to transfer{' '}
                            <b>
                    ...
                    <Table.Header>
                      <Table.Row>
                        <Table.HeaderCell colSpan="3">
                          You are currently allowed {symbol} by:
                        </Table.HeaderCell>
                      </Table.Row>
                      <Table.Row columns={2}>
...
export default TransferAllowed;
